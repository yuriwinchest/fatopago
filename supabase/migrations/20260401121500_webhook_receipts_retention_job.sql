-- Retenção automática de receipts de webhook do Mercado Pago.
-- Objetivo: evitar crescimento infinito da tabela mantendo janela auditável recente.
...
AS $$
DECLARE
  v_retention_days INTEGER := LEAST(GREATEST(COALESCE(p_retention_days, 45), 1), 365);
...
BEGIN
  ...
  -- Execução diária às 03:17 (horário do banco).
  PERFORM cron.schedule(
    'cleanup-mercadopago-webhook-receipts',
    '17 3 * * *',
    $job$select public.purge_mercadopago_webhook_receipts(45, 10000);$job$
  );
END;
$$;
