-- Fix type mismatch for admin_list_cycle_winners
...
BEGIN
  ...
  cycles AS (
    SELECT
      ...
      meta.is_active
    FROM offsets o
    CROSS JOIN LATERAL public.get_validation_cycle_meta(o.cycle_offset::INTEGER) AS meta
  ...
  LEFT JOIN LATERAL (
    SELECT rank_row.*
    FROM public.get_live_validation_ranking(NULL, NULL, 1, c.cycle_offset::INTEGER) AS rank_row
