-- =============================================================================
-- Migração: Notícias admin em TODOS os ciclos + desativar raspagem automática
-- =============================================================================
-- 1) admin_list_news_by_cycle  → lista TODAS as notícias admin (sem filtro de ciclo)
-- 2) submit_validation         → para admin posts, usa o ciclo ATUAL (não o da criação)
-- 3) Filtragem de raspagem     → será feita no frontend (is_admin_post = true)
-- =============================================================================

-- ─────────────────────────────────────────────────────────────────────────────
-- 1) admin_list_news_by_cycle: agora retorna TODAS as notícias admin
--    O p_cycle_offset é mantido na assinatura por compatibilidade mas ignorado.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2) submit_validation: para notícias admin, usar o ciclo ATUAL
--    em vez do cycle_start_at original da notícia.
--    Isso permite que notícias criadas em ciclos anteriores sejam validadas.
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- Para notícias admin: usar o ciclo ATUAL (permite validação em qualquer ciclo)
  -- Para notícias automáticas: manter comportamento original (ciclo da tarefa)
  IF v_is_admin_post THEN
    SELECT c.cycle_start_at
    INTO v_current_cycle_start
    FROM public.get_weekly_cycle_window(NOW(), 0) c;

    IF v_current_cycle_start IS NOT NULL THEN
      v_cycle_start := v_current_cycle_start;
    END IF;
  END IF;
  ...
  -- Duplicidade: para admin posts, verificar no ciclo ATUAL
  IF EXISTS (
    SELECT 1
