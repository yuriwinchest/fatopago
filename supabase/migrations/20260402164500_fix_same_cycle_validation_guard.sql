-- Corrige a trava de duplicidade de validacao.
-- Regra correta:
-- 1) o usuario nao pode validar a mesma noticia no mesmo ciclo
-- 2) o mesmo task_id pode voltar a ser validado em ciclos futuros
-- 3) a verificacao deve respeitar a janela do ciclo da tarefa

DROP INDEX IF EXISTS public.validations_user_task_unique_idx;

CREATE INDEX IF NOT EXISTS validations_user_task_cycle_lookup_idx
  ON public.validations (user_id, task_id, created_at DESC)
