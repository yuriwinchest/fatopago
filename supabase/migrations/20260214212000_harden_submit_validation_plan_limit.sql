-- Harden submit_validation to strictly enforce plan validation limits even under concurrency.
-- Fixes a race condition where parallel requests could exceed max_validations.
...
BEGIN
  ...
  -- Prevent double voting in the same cycle
  SELECT EXISTS (
    SELECT 1
