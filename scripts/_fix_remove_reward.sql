-- Remove reward logic from submit_validation
-- Validation only CONSUMES plan credit, does NOT generate balance
-- User balance only comes from referral commissions
...
BEGIN
  ...
  INTO v_category, v_task_created, v_task_closed, v_task_status
  ...
  INTO v_user_email, v_user_name, v_user_lastname, v_user_city, v_user_state, v_user_phone
  ...
  INSERT INTO public.profiles (
    ...
    v_user_id, COALESCE(v_user_name, 'Usuário'), COALESCE(v_user_lastname, ''),
    COALESCE(v_user_city, ''), COALESCE(v_user_state, ''),
    0, 0, 0, TRUE, NOW(), NOW(), v_user_email, v_user_phone, 'none'
  )
  ...
  IF v_plan_debit > 0.009 THEN
    SELECT * INTO v_plan FROM public.plan_purchases
    ...
    IF NOW() >= v_plan_expires_at THEN
      UPDATE public.plan_purchases SET status = 'completed', completed_at = COALESCE(completed_at, NOW()), updated_at = NOW() WHERE id = v_plan.id;
  ...
  -- Duplicate check: por plan_purchase_id (nao por ciclo)
  IF v_plan_id IS NOT NULL AND EXISTS (
    SELECT 1 FROM public.validations
    WHERE user_id = v_user_id AND task_id = p_task_id AND plan_purchase_id = v_plan_id
  ) THEN
  ...
  VALUES (p_task_id, v_user_id, v_plan_id, p_verdict, v_justification, v_proof_link)
  RETURNING id INTO v_validation_id;

  -- Update compensatory credit
  IF v_comp_used > 0 THEN
    UPDATE public.profiles SET compensatory_credit_balance = v_comp_remaining_after, updated_at = NOW() WHERE id = v_user_id;
  END IF;

  -- Update plan credit (consume news from package)
  IF v_plan_debit > 0.009 THEN
    v_plan_remaining_after := GREATEST(COALESCE(v_plan.validation_credit_remaining, 0) - v_plan_debit, 0);
    UPDATE public.plan_purchases SET
      used_validations = COALESCE(used_validations, 0) + 1,
      validation_credit_remaining = v_plan_remaining_after,
      last_validation_at = NOW(), updated_at = NOW(),
  ...
  -- Update reputation only (no balance reward)
  UPDATE public.profiles SET
    reputation_score = COALESCE(reputation_score, 0) + 10,
    updated_at = NOW()
  WHERE id = v_user_id;

  -- Debit ledger entry (plan consumption)
  INSERT INTO public.financial_ledger (
    user_id, entry_type, amount, source_table, source_id,
    description, transaction_status, actor_user_id, metadata
  ) VALUES (
    v_user_id, 'debit', v_cost, 'validations', v_validation_id::TEXT,
    'Consumo de notícia do pacote', 'completed', v_user_id,
    jsonb_build_object(
      'task_id', p_task_id, 'plan_purchase_id', v_plan_id, 'verdict', p_verdict,
      'compensatory_credit_used', v_comp_used, 'plan_credit_used', v_plan_debit,
...
-- Fix: zero out incorrectly credited balances from the old reward bug
-- and remove the bogus reward transactions
UPDATE profiles SET current_balance = 0, updated_at = NOW()
WHERE current_balance > 0;

DELETE FROM transactions
WHERE type = 'credit' AND description = 'Recompensa: validação de notícia';

-- financial_ledger is append-only; insert a reversal entry instead
INSERT INTO financial_ledger (user_id, entry_type, amount, source_table, source_id, description, transaction_status, actor_user_id, metadata)
SELECT fl.user_id, 'debit', fl.amount, 'financial_ledger', 'reversal_' || fl.id::TEXT,
       'Estorno: recompensa por validação removida', 'completed', fl.user_id,
       jsonb_build_object('original_ledger_id', fl.id, 'reason', 'reward_logic_removed')
FROM financial_ledger fl
WHERE fl.entry_type = 'credit' AND fl.description = 'Recompensa por validação de notícia'
ON CONFLICT (source_table, source_id) DO NOTHING;
