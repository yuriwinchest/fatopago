-- Fix: validation per plan_purchase (not per cycle)
-- Fix: remove cycle_end block (news persist across cycles)
-- Fix: keep reward logic

-- Drop old cycle-based index
DROP INDEX IF EXISTS public.validations_user_task_cycle_lookup_idx;

-- Create per-plan-purchase uniqueness index
CREATE UNIQUE INDEX IF NOT EXISTS validations_user_task_plan_unique_idx
...
BEGIN
  ...
  -- Fetch task (sem bloqueio por ciclo - noticias persistem entre ciclos)
  SELECT
    content->>'category',
    created_at,
  ...
  -- Usuario pode revalidar a mesma noticia se comprar novo pacote
  ...
  -- Credit reward to balance + reputation
  ...
  -- Credit transaction for user visibility
