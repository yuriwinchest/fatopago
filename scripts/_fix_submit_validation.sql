-- FIX 1: Add RLS policy on transactions for users
DO $$
BEGIN
  IF NOT EXISTS (
...
-- FIX 2: Update submit_validation to credit balance + reputation + insert transaction
...
BEGIN
  ...
  INTO v_category, v_cycle_start, v_task_created, v_task_closed, v_task_status
  ...
  -- *** Credit reward to balance + update reputation ***
  ...
  -- Insert debit ledger entry (plan consumption)
  ...
  -- Insert credit ledger entry (validation reward)
  INSERT INTO public.financial_ledger (
    user_id, entry_type, amount, source_table, source_id,
    description, transaction_status, actor_user_id, metadata
  ) VALUES (
    v_user_id, 'credit', v_reward, 'validations', 'reward_' || v_validation_id::TEXT,
    'Recompensa por validação de notícia', 'completed', v_user_id,
    jsonb_build_object('task_id', p_task_id, 'validation_id', v_validation_id, 'reward', v_reward)
  ) ON CONFLICT (source_table, source_id) DO NOTHING;

  -- Insert credit transaction for user visibility
  INSERT INTO public.transactions (user_id, type, amount, description, status)
  VALUES (v_user_id, 'credit', v_reward, 'Recompensa: validação de notícia', 'completed');
...
-- FIX 3: Backfill existing users reputation and balance
WITH user_stats AS (
  SELECT user_id, COUNT(*) AS total_validations
  FROM validations
  GROUP BY user_id
)
UPDATE profiles p
SET
  reputation_score = COALESCE(p.reputation_score, 0) + (us.total_validations * 10),
  current_balance = COALESCE(p.current_balance, 0) + (us.total_validations * 1.00),
  updated_at = NOW()
FROM user_stats us
WHERE p.id = us.user_id;

-- FIX 4: Backfill reward transactions for existing validations
INSERT INTO transactions (user_id, type, amount, description, status, created_at)
SELECT v.user_id, 'credit', 1.00, 'Recompensa: validação de notícia', 'completed', v.created_at
FROM validations v
WHERE NOT EXISTS (
  SELECT 1 FROM transactions t
  WHERE t.user_id = v.user_id AND t.type = 'credit'
    AND t.description = 'Recompensa: validação de notícia'
